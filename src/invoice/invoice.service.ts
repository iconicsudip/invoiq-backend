...
import { CreateInvoiceDto, MarkPaidDto, UpdateInvoiceDto } from "./dto";
...
export class InvoiceService {
  ...
  async create(workspaceId: string, dto: CreateInvoiceDto) {
    ...
    // Calculate totals
    const { subtotal, totalGst, cgst, sgst, igst, total, items } =
  ...
  async update(id: string, workspaceId: string, dto: UpdateInvoiceDto) {
      ...
      await this.prisma.$transaction([
        ...
        this.prisma.invoice.update({
          ...
          data: {
            ...
            totalGst,
            total,
            amountDue: total - (inv.amountPaid as any),
          },
        }),
        ...items.map((item) =>
          this.prisma.invoiceItem.create({ data: { ...item, invoiceId: id } }),
        ),
      ]);
    ...
    return this.prisma.invoice.update({
      where: { id },
      data: { ...dto, items: undefined } as any,
    });
  }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (inv.projectId) {
        await tx.project.update({
          ...
          data: { totalRevenue: { increment: inv.total as any } },
        });
      }
  ...
  async duplicate(id: string, workspaceId: string) {
    ...
    const dto: CreateInvoiceDto = {
      ...
      items: (original.items as any[]).map((i) => ({
        itemName: i.itemName,
  ...
  private calculateTotals(items: any[], discountPercent: number) {
    ...
    const totalGst = processed.reduce(
